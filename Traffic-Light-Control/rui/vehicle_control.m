...
path(1,1,:)=[1.2 1.8];path(1,2,:)=[2.454 1.8];path(1,3,:)=[0 0];%define path1. [0 0] is stopping mark 
...
while(1)
  ...
  if(path(pathnum(str2double(CARID)),current(str2double(CARID)),1)~=0&&path(pathnum(str2double(CARID)),current(str2double(CARID)),2)~=0)
    ...
    if dist > 0.09 && reachTarget(str2double(CARID)) == 0
...
result=1; 
 for j=1:1:num_pololu
    if(str2double(CARID)~=j)
        if(norm([xc(str2double(CARID)), yc(str2double(CARID))]-[xc(j), yc(j)])<0.31)
            if START(str2double(CARID))==0
                if START(j)==0
                y2=path(pathnum(str2double(CARID)),2,2)-yc(str2double(CARID));
                y1=path(pathnum(str2double(CARID)),2,2)-yc(j);
                x2=path(pathnum(str2double(CARID)),2,1)-xc(str2double(CARID));
                x1=path(pathnum(str2double(CARID)),2,1)-xc(j);
            ...
            if  (START(j)==0 && START(str2double(CARID))==pathnum(j))||(START(str2double(CARID))==0&&START(j)~= pathnum(str2double(CARID))&&START(j)~=0)
            else
